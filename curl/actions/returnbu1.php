        ...
        <div class="container" style="min-height: 100%">
            <div class="row">
                <div class="text-center"><br>
                    ...
                    $q = $quantity + 1;
                    $sqlu1="UPDATE student "." SET d1 = '', b1 = '', fd1='',fine='$fine' "." WHERE adm ='$adm'" ;
                    $resultu1 = mysqli_query($conn, $sqlu1);
                    $sql2 = "UPDATE Books SET status = ('" . $q . "') WHERE bookid = ('".$bid."')";
                    $result2 = mysqli_query($conn, $sql2);
                    if($fine>0)
                    {
                        echo '<form action="../librarian/clear.php" method="post">';
                        echo '<input type="hidden" value="'.$adm.'" name="uname">';
                        echo '<input type="hidden" value="0" name="clear">';
                        echo '<input type="submit" class="subtn" style="background-color: transparent;padding:4%" value="Clear dues">';
                    }
                    else{
                        echo '<p onload="pageRedirect()">No dues</p>';
                    }
                    ?>
